		...
		update: function(title,content,tip,tableElem,data) {
			if(!data){
				...
				if(checkStatus.data.length > 1) {
			         layer.msg("当前只能修改一条数据",{time:5*1000});
			         return;
			         }
			...
			if(data === null || data === undefined){
				layer.msg("请选择需要修改的内容",{time:5*1000});
				return false;
			}
		...
		deleteRow: function(tableElem,url, id, data) {
			if(!data){
				...
				if(checkStatus.data.length == 0) {
			         layer.msg("当前只能选择一条需要删除的内容",{time:5*1000});
			         return;
			     }
				data = checkStatus.data;
			}
			if(data === null || data === undefined){
				layer.msg("请选择需要删除的内容",{time:5*1000});
				return false;
			}
	      	var newsId = [];
     		...
     		layui.comExt.ajax({
           		...
           		error:function(e){
           			...
           			layer.close(index);	
           			top.layer.msg("删除失败！错误码："+e.status,{time:5*1000});
           			return ; 
           		}
           	});
           });
